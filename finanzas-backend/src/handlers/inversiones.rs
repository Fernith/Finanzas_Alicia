...
pub async fn listar_inversiones(
    ...
    let rows = sqlx::query!(
        ...
        SELECT 
            ...
            c.id::text as "categoria_id!",
            c.nombre as "categoria!",
            cu.id::text as "cuenta_id!",
            cu.nombre as "cuenta!",
            o.descripcion,
            c.color as "color!",
        ...
        WHERE o.tipo_operacion_id = 'INVERSION'::tipo_operacion_enum
          AND (
              ($3::text != '' AND (
...
pub async fn obtener_categorias_inversiones(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows = sqlx::query_as!(
        MaestroDTO,
        r#"SELECT id::text as "id!", nombre, color, activo FROM categorias WHERE tipo_operacion_id = 'INVERSION'::tipo_operacion_enum ORDER BY orden ASC, nombre ASC"#
    )
    .fetch_all(&pool)
    .await;
...
pub async fn obtener_cuentas_inversiones(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows = sqlx::query_as!(
        ...
        r#"SELECT c.id::text as "id!", c.nombre, c.color, c.activo 
           ...
           WHERE ct.tipo_operacion_id = 'INVERSION'::tipo_operacion_enum
...
pub async fn crear_inversion(State(pool): State<PgPool>, Json(payload): Json<UpsertInversionDTO>) -> impl IntoResponse {
    let result = sqlx::query(
        r#"INSERT INTO operaciones (tipo_operacion_id, fecha, cantidad, categoria_id, cuenta_id, descripcion, pendiente)
           VALUES ('INVERSION'::tipo_operacion_enum, $1::text::date, $2::float8::numeric, $3::text::uuid, $4::text::uuid, $5, $6::boolean)"#
    )
    .bind(&payload.fecha)
    ...
    match result {
        Ok(_) => (StatusCode::CREATED, "Inversion creada").into_response(),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response(),
    }
}

pub async fn modificar_inversion(
    State(pool): State<PgPool>,
    Path(id): Path<String>,
    Json(payload): Json<UpsertInversionDTO>,
) -> impl IntoResponse {
    let result = sqlx::query(
        r#"UPDATE operaciones 
           ...
           WHERE id = $7::text::uuid AND tipo_operacion_id = 'INVERSION'::tipo_operacion_enum"#
    )
    .bind(&payload.fecha)
    .bind(payload.cantidad)
    .bind(&payload.categoria_id)
    ...
    match result {
        Ok(_) => (StatusCode::OK, "Inversion actualizada").into_response(),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response(),
    }
}

pub async fn eliminar_inversion(State(pool): State<PgPool>, Path(id): Path<String>) -> impl IntoResponse {
    let result = sqlx::query!("DELETE FROM operaciones WHERE id = $1::text::uuid AND tipo_operacion_id = 'INVERSION'::tipo_operacion_enum", id)
        .execute(&pool)
        .await;

    match result {
        Ok(_) => (StatusCode::OK, "Inversion eliminada").into_response(),
        Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response(),
    }
}
