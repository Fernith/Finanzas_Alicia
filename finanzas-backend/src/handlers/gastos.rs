...
pub async fn obtener_cuentas_gastos(State(pool): State<PgPool>) -> impl IntoResponse {
    let rows = sqlx::query_as!(
        ...
        r#"SELECT c.id::text as "id!", c.nombre, c.color, c.activo 
           ...
           WHERE ct.tipo_operacion_id = 'GASTO'::tipo_operacion_enum
           ORDER BY c.orden ASC, c.nombre ASC"#
    ).fetch_all(&pool).await;
...
pub async fn crear_gasto(State(pool): State<PgPool>, Json(payload): Json<NuevoGastoDTO>) -> impl IntoResponse {
    let result = sqlx::query(
        r#"INSERT INTO operaciones (tipo_operacion_id, fecha, cantidad, categoria_id, cuenta_id, descripcion, pendiente)
           VALUES ('GASTO'::tipo_operacion_enum, $1::text::date, $2::float8::numeric, $3::text::uuid, $4::text::uuid, $5, $6::boolean)"#
    )
    ...
    match result { Ok(_) => (StatusCode::CREATED, "Gasto creado").into_response(), Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response() }
}
...
pub async fn actualizar_gasto(State(pool): State<PgPool>, Path(id): Path<String>, Json(payload): Json<NuevoGastoDTO>) -> impl IntoResponse {
    let result = sqlx::query(
        r#"UPDATE operaciones 
           ...
           WHERE id = $7::text::uuid AND tipo_operacion_id = 'GASTO'::tipo_operacion_enum"#
    )
    .bind(&payload.fecha).bind(payload.cantidad).bind(&payload.categoria_id)
    .bind(&payload.cuenta_id).bind(&payload.descripcion).bind(payload.pendiente).bind(&id).execute(&pool).await;

    match result { Ok(_) => (StatusCode::OK, "Gasto actualizado correctamente").into_response(), Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response() }
}
...
pub async fn eliminar_gasto(State(pool): State<PgPool>, Path(id): Path<String>) -> impl IntoResponse {
    ...
    match result { Ok(_) => (StatusCode::OK, "Gasto eliminado físicamente").into_response(), Err(err) => (StatusCode::INTERNAL_SERVER_ERROR, err.to_string()).into_response() }
}
