...
export default function Ajustes() {
  ...
  useEffect(() => { cargarCategorias(); cargarCuentas(); }, []);
  ...
  const ejecutarAccionConfirmada = async () => {
    ...
    try {
      ...
      if (res.ok) { target === 'categorias' ? cargarCategorias() : cargarCuentas(); } 
      else { alert(`No se pudo ${tipo} el elemento.`); }
  ...
  const confirmarTogglePendientes = async () => {
    ...
    try {
      await fetch('/api/configuracion', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ usar_pendientes: nuevoEstado }) });
    } catch (e) {
      alert('Error de conexión al guardar la configuración'); setUsarPendientes(!nuevoEstado); 
  ...
  const handleDrop = async (e: React.DragEvent, dropIndex: number, target: string) => {
    ...
    if (isCategoria) setCategorias(items); else setCuentas(items);

    const payload = items.map((item, idx) => ({ id: item.id, orden: idx }));
    try {
      const res = await fetch(`/api/ajustes/${target}/reordenar`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      if (!res.ok) throw new Error('Error al guardar');
    } catch {
      alert("Error al guardar el nuevo orden en el servidor.");
      isCategoria ? cargarCategorias() : cargarCuentas();
    } finally { setIsSavingOrder(false); setDraggedItem(null); }
  };
  ...
  const renderColumnaMaestra = (titulo: string, items: any[], target: 'categorias' | 'cuentas') => (
    <div className={`bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-700 rounded-2xl shadow-sm flex flex-col overflow-hidden h-full transition-opacity ${isSavingOrder ? 'opacity-70 pointer-events-none' : ''}`}>
      <div className="p-5 border-b border-slate-200 dark:border-neutral-700 bg-slate-50/50 dark:bg-neutral-900/50 flex justify-between items-center">
        ...
        <button onClick={() => handleAbrirAlta(target)} className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-500 hover:bg-blue-600 text-white text-xs font-semibold rounded-lg transition-colors shadow-sm shadow-blue-500/10 active:scale-95"><Plus size={14} /> Añadir</button>
      </div>
      
      <div className="divide-y divide-slate-100 dark:divide-neutral-800 overflow-y-auto max-h-[500px]">
        {items.length > 0 ? (
          items.map((item, index) => (
            <div key={item.id} draggable onDragStart={(e) => handleDragStart(e, index, target)} onDragOver={(e) => handleDragOver(e, index, target)} onDragLeave={() => setDragOverIndex(null)} onDrop={(e) => handleDrop(e, index, target)}
              className={`p-4 flex justify-between items-center transition-all cursor-move ${!item.activo ? 'opacity-50 bg-slate-50/40 dark:bg-neutral-900/20' : 'hover:bg-slate-50/30 dark:hover:bg-neutral-800/50'} ${draggedItem?.target === target && dragOverIndex === index ? 'bg-slate-100 dark:bg-neutral-800 shadow-inner' : ''}`}
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex gap-1 mt-1 flex-wrap">
                    {typeof item.tipo_operacion_id === 'string' && (<span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${item.tipo_operacion_id === 'GASTO' ? 'bg-red-50 text-red-600 dark:bg-red-950/30' : item.tipo_operacion_id === 'INGRESO' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30' : 'bg-amber-50 text-amber-600 dark:bg-amber-950/30'}`}>{item.tipo_operacion_id}</span>)}
                    {Array.isArray(item.tipos_operacion) && item.tipos_operacion.map((tipo: string) => (<span key={tipo} className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${tipo === 'GASTO' ? 'bg-red-50 text-red-600 dark:bg-red-950/30' : tipo === 'INGRESO' ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-950/30' : 'bg-amber-50 text-amber-600 dark:bg-amber-950/30'}`}>{tipo}</span>))}
                  </div>
                </div>
              </div>

              <div className="flex items-center gap-2">
                {item.activo ? (<span className="text-emerald-500" title="Activo"><CheckCircle size={14} /></span>) : (<span className="text-slate-400 flex items-center gap-1 text-[11px] font-medium" title="Inactivo"><XCircle size={14} /> Inactivo</span>)}
                ...
                {item.activo ? (<button onClick={() => setAccionConfirmacion({ target, id: item.id, nombre: item.nombre, tipo: 'desactivar' })} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 rounded-lg transition-colors" title="Desactivar"><Trash2 size={15} /></button>) : (<button onClick={() => setAccionConfirmacion({ target, id: item.id, nombre: item.nombre, tipo: 'activar' })} className="p-1.5 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/20 rounded-lg transition-colors" title="Reactivar"><RotateCcw size={15} /></button>)}
              </div>
            </div>
          ))
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 border-b border-slate-200 dark:border-neutral-700 pb-6">
        <div className="flex items-center gap-4">
          {/* 3. Contenedor e icono de la tuerca en tonos grises */}
      ...
      <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-700 rounded-2xl p-6 shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Activa esta opción para registrar operaciones que aún no se han reflejado en tu banco.</p>
        </div>
      ...
      <ModalConfirmacion isOpen={!!accionConfirmacion} onClose={() => setAccionConfirmacion(null)} onConfirm={ejecutarAccionConfirmada} titulo={accionConfirmacion?.tipo === 'desactivar' ? "Desactivar elemento" : "Reactivar elemento"} mensaje={accionConfirmacion?.tipo === 'desactivar' ? `¿Estás seguro de que deseas desactivar "${accionConfirmacion?.nombre}"? Dejará de aparecer en los selectores.` : `Vas a reactivar "${accionConfirmacion?.nombre}". Volverá a estar disponible para nuevos registros.`} textoBoton={accionConfirmacion?.tipo === 'desactivar' ? "Desactivar" : "Reactivar"} variante={accionConfirmacion?.tipo === 'desactivar' ? 'danger' : 'success'} />
      <ModalConfirmacion isOpen={modalPendientesAbierto} onClose={() => setModalPendientesAbierto(false)} onConfirm={confirmarTogglePendientes} titulo={usarPendientes ? "Desactivar Operaciones Pendientes" : "Activar Operaciones Pendientes"} mensaje={usarPendientes ? "¿Estás seguro de que deseas desactivar la gestión de operaciones pendientes? Dejarán de mostrarse las operaciones como pendientes en los listados y las sumas se calcularán sobre el total de registros." : "¿Estás seguro de que deseas activar la gestión de operaciones pendientes? Podrás registrar movimientos que aún no se han reflejado en el banco."} textoBoton={usarPendientes ? "Desactivar" : "Activar"} variante={usarPendientes ? 'danger' : 'success'} />
    </div>
  );
}